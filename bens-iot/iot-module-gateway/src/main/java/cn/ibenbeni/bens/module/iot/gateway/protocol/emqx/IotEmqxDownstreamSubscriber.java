...
import cn.ibenbeni.bens.module.iot.gateway.protocol.emqx.router.IotEmqxDownstreamHandler;
...
public class IotEmqxDownstreamSubscriber implements IotMessageSubscriber<IotDeviceMessage> {
    ...
    public void onMessage(IotDeviceMessage message) {
        log.debug("[onMessage][接收到下行消息, messageId: {}, method: {}, deviceId: {}]", message.getMsgId(), message.getMethod(), message.getDeviceId());
        try {
            // 1.校验参数
            String method = message.getMethod();
            if (method == null) {
                log.warn("[onMessage][消息方法为空, messageId: {}, deviceId: {}]", message.getMsgId(), message.getDeviceId());
                return;
            }
            ...
            log.error("[onMessage][处理下行消息失败, messageId: {}, method: {}, deviceId: {}]", message.getMsgId(), message.getMethod(), message.getDeviceId(), ex);
        }
    }
