...
(
    ...
    `sys_flag`         char(1) NULL DEFAULT 'Y' COMMENT '是否是系统参数：Y=是，N=否',
    `config_sort`      decimal(10, 2) NOT NULL COMMENT '排序',
    `remark`           varchar(255) NULL DEFAULT NULL COMMENT '备注',
...
(
    ...
    `config_type_code` varchar(100)   NOT NULL COMMENT '参数配置类型编码',
    `sys_flag`         char(1) NULL DEFAULT 'Y' COMMENT '是否为系统参数：Y=是，N=否',
    `config_type_sort` decimal(10, 2) NOT NULL COMMENT '排序',
    `remark`           varchar(255) NULL DEFAULT NULL COMMENT '备注',
    `del_flag`         char(1)        NOT NULL DEFAULT 'N' COMMENT '是否删除：Y=被删除，N=未删除',
    `create_time`      datetime(0) NULL DEFAULT NULL COMMENT '创建时间',
    `create_user`      bigint NULL DEFAULT NULL COMMENT '创建人',
